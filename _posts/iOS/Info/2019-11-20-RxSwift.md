...
- cold observable 을  hot으로 바꿀 수 있다.
...
- observable에서 제공
- onNext사용가능
- onError, onComplete없음. UI에서사용하기좋은

## Observable - Driver | Subject - Relay

### Driver

- 에러없는 Observable
- subscribe 대신 drive 사용
...
- 에러와 완료없는 Subject
- UI작업에 적합
- relay로 데이이터 전달은  accept만 가능  (subject는 .next, .error, .completed )
...
- RxCocoa에 정의
- Observable은 생성된 쓰레드에 영향을 받음
- Driver는 메인 스케줄러에서 동작
- 에러없는 Observable
- 구독 시 subscribe 대신 drive 사용
...
Observable, Relay, Driver 모두 bindTo사용 가능
``` swift
var text: Observable<String>
var currentReadText: Observable<NSRange>
var test: PublishRelay<String>?
var test2: Driver<String>?
```
